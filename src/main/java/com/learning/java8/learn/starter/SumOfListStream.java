...
public class SumOfListStream {
	
	public static void main(String[] args) {
		List<Integer> numbers = Arrays.asList(1, 2, 4, 5, 5, 6);
		Integer sum = numbers.stream().reduce(0,(a,b)->a+b);
		System.out.println("Sum is " +sum);
		Integer sumUSingMethodReffernce = numbers.stream().reduce(0,SumOfListStream::add);
       ...
       int sumUsingIntStreamM = numbers.stream().mapToInt(SumOfListStream::convert).sum();
